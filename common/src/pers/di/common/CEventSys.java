...
public class CEventSys {
	...
	{
		CLog.output("", "BL_EventSys Initialized...\n");
		try {
			// init context
	...
	{
		CLog.output("EVENT", "BL_EventSys UnInitialize...\n");
	...
	public static class EventSender
	{
		...
		public boolean Send(String name, JSONObject jsonObj)
		{
			...
			if(null == jsonObj) 
			{
				CLog.output("EVENT", "Sender EvName(%s) jsonObj(null)\n", name);
				s_PubSocket.send(name.getBytes(), ZMQ.SNDMORE); 
				s_PubSocket.send("null".getBytes(), 0);
			}
			else
			{
				CLog.output("EVENT", "Sender EvName(%s) Data(...)\n", name);
				s_PubSocket.send(name.getBytes(), ZMQ.SNDMORE); 
	...
	public static class EventReceiver
	{
		...
		private static class ReceiverThread extends Thread
		{
	        ...
	        public void run()
	        {
				CLog.output("EVENT", "EventReceiver(%s) thread running.\n",  m_receiver.m_ReceiverName);
				while(!m_receiver.m_Quit){
					...
					CLog.output("EVENT", "Receiver(%s) EvName(%s) jsonObj(...)\n", m_receiver.m_ReceiverName, name);
				...
				CLog.output("EVENT", "EventReceiver(%s) thread exit!\n", m_receiver.m_ReceiverName);
	        }
		...
		public boolean Subscribe(String name, Object obj, String methodname)
		{
			try {
				...
				CLog.output("EVENT", "%s Subscribe %s\n", m_ReceiverName, name);
			} catch (Exception e) {
				e.printStackTrace();
			}
			return true;
		...
		{
			CLog.output("EVENT", "EventReceiver(%s) startReceive\n", m_ReceiverName);
		...
		{
			CLog.output("EVENT", "EventReceiver(%s) stopReceive\n", m_ReceiverName);
